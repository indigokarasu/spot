...
- VPN Gate config at `~/vpn_gate.ovpn`
...
openvpn --config ~/vpn_gate.ovpn --daemon --log ~/openvpn.log
...
if 'data-ciphers' not in config:
    ...
    for i, line in enumerate(lines2):
        if line.startswith('cipher '):
            ...
            break
    config = '\n'.join(lines2)
with open('~/vpn_gate.ovpn', 'w') as f:
    f.write(config)
print(f"Selected: {target['CountryLong']} {target['IP']}")
EOF

# Reconnect
openvpn --config ~/vpn_gate.ovpn --daemon
sleep 5
...
- If tun0 is down, reconnect VPN: `openvpn --config ~/vpn_gate.ovpn --daemon`
